# Serras Gaúchas 2026: O Planejamento

**Status:** :material-calendar: Planejamento
**Data:** 19/01/2026 a 02/02/2026 (15 dias)
**Moto:** Triumph Tiger 900 Rally Pro

---

## O Roteiro

Esta expedição de 15 dias tem como foco as lendárias estradas das serras do Rio Grande do Sul e Santa Catarina. O objetivo não é apenas chegar, mas aproveitar cada curva.

- [:material-calendar: Ver Roteiro Detalhado (Dia a Dia)](roteiro.md)
- [:material-checkbox-marked-outline: Ver Checklist de Viagem](checklist.md)
- [:material-map-marker: Mapas Offline](mapas-offline.md)

---

## Trechos Principais

| Fase | Dias | Destaque |
|------|------|----------|
| Deslocamento | 1-4 | Goiânia → Urubici |
| Serras SC | 5-6 | Corvo Branco, Rio do Rastro |
| Cânions | 7-8 | Itaimbezinho, Fortaleza |
| Vinhedos | 9-10 | Vale dos Vinhedos |
| Retorno | 11-15 | Via Curitiba (dia de folga) |

---

## Destaques do Trajeto

- :material-mountain: **Serra do Rio do Rastro:** O teste definitivo de freios e vertigem.
- :material-bird: **Serra do Corvo Branco:** Estradas estreitas e visuais dramáticos.
- :material-terrain: **Cânions de Cambará do Sul:** Itaimbezinho e Fortaleza.
- :material-bottle-wine: **Vale dos Vinhedos:** Um pouco de conforto e gastronomia.

---

## Alertas Importantes

!!! danger "Serra do Corvo Branco"
    Travessia **FECHADA até março/2026**. Acesso ida/volta por Urubici liberado.

!!! warning "Tirolesa Fortaleza"
    **FECHADA desde maio/2024**

---

## Custos Estimados

O orçamento foi calculado para uma viagem confortável, mas consciente.

| Categoria | Custo Estimado (R$) | Notas |
| :--- | :--- | :--- |
| **Hospedagem** | R$ 3.500 | Média de R$ 250/noite |
| **Alimentação** | R$ 2.000 | R$ 135/dia |
| **Combustível** | R$ 1.200 | Baseado no consumo médio da Tiger |
| **Pedágios** | R$ 300 | Principalmente em SP |
| **Ingressos/Passeios** | R$ 500 | Parques, vinícolas |
| **Extras** | R$ 500 | Emergências, souvenirs |
| **TOTAL** | **R$ 8.000** | |

---

## Guias Complementares

- [:material-hospital-box: Guia de Emergências](guia-emergencias.md) - Hospitais, mecânicas, contatos
- [:material-silverware-fork-knife: Guia Gastronômico](guia-gastronomico.md) - Onde comer em cada região
- [:material-weather-partly-cloudy: Guia de Clima](guia-clima.md) - Previsões e o que vestir
- [:material-tools: Manutenção em Viagem](manutencao-viagem.md) - Cuidados com a moto

---

## Arquivos GPX

Os arquivos de rota detalhados estão disponíveis para download:

| Arquivo | Descrição |
|---------|-----------|
| `01_Rota_Cenica_Completa.gpx` | Roteiro completo de 15 dias |
| `02_Rota_Ida_Cenica.gpx` | Trecho Goiânia → Urubici |
| `03_Rota_Volta_Asfaltada.gpx` | Retorno rápido (padrão) |
| `04_Rota_Volta_Cenica.gpx` | Retorno cênico (alternativa) |

### Como usar os GPX

=== "Garmin"
    Copie os arquivos `.gpx` para `/Garmin/GPX/` e sincronize com BaseCamp.

=== "My Triumph App"
    1. Baixe os arquivos GPX
    2. Importe para o My Triumph App
    3. Sincronize via Bluetooth antes da partida

!!! info "Dica Técnica"
    Sempre mantenha os mapas offline do Google Maps baixados para a região, pois o sinal de celular é inexistente em vários trechos da serra.

    [Confira o guia passo a passo para baixar mapas offline](mapas-offline.md).

---

## Rotas de Retorno

Você tem **duas opções** de retorno a partir de Bento Gonçalves:

### Opção 1: Retorno Rápido (Padrão)

```
Bento → Vacaria → Curitiba → Ourinhos → Uberaba → Goiânia
```

- ~1.570 km em 5 dias
- Estradas melhores, mais rápido

### Opção 2: Retorno Cênico

```
Bento → São José dos Ausentes → Vacaria → Lages → Curitiba → Goiânia
```

- ~1.700 km em 5 dias
- Paisagens incríveis, revisita a serra

**Decida durante a viagem conforme seu estado de ânimo!**

---

*Última atualização: Dezembro/2025*
